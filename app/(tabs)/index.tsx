...
function ProductMedia({ imageUrl, name, color }: { imageUrl: string | null; name: string; color: string }) {
  const [failed, setFailed] = useState(false);
  if (!imageUrl || failed) return <ProductPlaceholder name={name} color={color} />;
  return <Image source={{ uri: imageUrl }} onError={() => setFailed(true)} style={{ width: "100%", height: "100%" }} resizeMode="cover" />;
}
