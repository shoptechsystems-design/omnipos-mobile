...
import { getPortalSessionCookie, portal } from "@/lib/portal";
...
function ProductMedia({ imageUrl, imageProxyUrl, name, color }: { imageUrl: string | null; imageProxyUrl?: string | null; name: string; color: string }) {
  ...
  useEffect(() => {
    setSource(imageUrl);
    setFailed(false);
    getPortalSessionCookie().then(setCookie).catch(() => setCookie(null));
  }, [imageUrl]);
  if (!source || failed) return <ProductPlaceholder name={name} color={color} />;
  return <Image source={{ uri: source, headers: cookie ? { Cookie: cookie } : undefined }} onError={() => {
